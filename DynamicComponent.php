...
class DynamicComponent extends Component
{
     ...
     *
     * @var \QuantaQuirk\View\Compilers\BladeTagCompiler
     */
     ...
     *
     * @return \QuantaQuirk\Contracts\View\View|string
     */
    public function render()
    {
        $template = <<<'EOF'
<?php extract(collect($attributes->getAttributes())->mapWithKeys(function ($value, $key) { return [QuantaQuirk\Support\Str::camel(str_replace([':', '.'], ' ', $key)) => $value]; })->all(), EXTR_SKIP); ?>
{{ props }}
...
EOF;
        ...
        return function ($data) use ($template) {
            ...
            return str_replace(
                ...
                [
                    ...
                    $this->compileSlots($data['__quantaquirk_slots']),
                    '{{ $slot ?? "" }}',
                ],
                $template
            );
     ...
     *
     * @return \QuantaQuirk\View\Compilers\ComponentTagCompiler
     */
